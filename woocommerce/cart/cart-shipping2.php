	...
	<ul class="steps-checkout__list">
		...
		<?php foreach ( $available_methods as $method ) :
		    ...
		    if($method->id == 'local_pickup:1'){?>
		        ...
		        <li>
					<label class="checkbox-radio checkbox-radio--radio checkbox-radio--xs chekin">

				    <?php printf( '<input type="radio" name="shipping_method[%1$d]" class="shipping_method" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" %4$s />', $index, esc_attr( sanitize_title( $method->id ) ), esc_attr( $method->id ), checked( $method->id, $chosen_method, false ) );?>
		...
		<li>
			<label class="checkbox-radio checkbox-radio--radio checkbox-radio--xs chekin" data-collapse-trigger="1">
				<input type="radio" name="delivery-method" <?php if(WC()->session->get('chosen_shipping_methods')[0] !== 'local_pickup:1'){echo 'checked';}?>>
				<div class="checkbox-radio__square"></div>
